...
install_xmlPath() {
    ...
    print_message info "-------------------------------------"
    echo
    sudo yum install perl-XML-XPath -y
    echo
}
...
save_package_details() {
    ...
    print_message info "User $type packages: "
    echo "$user_packages_json"
...
check_package_version() {
    type=$1
    print_message info "package type: $type"
    xml_tag=$2
    print_message info "xml tag: $xml_tag"
    expression=$3
    print_message info "expression: $expression"
    xml_path=$4
    print_message info "xml path: $xml_path"
    count_variable=$5
    print_message info "Count Variable: $count_variable"
    ...
    then
        ...
        COUNT=$(echo "$xml_required" | xpath "count($xml_path)" 2>/dev/null)

        print_message info "Count of required tags present: $COUNT"
        
        # Iterating over all the tags
        for (( i=1; i<=COUNT; i++ ))
        do
            GROUPID=$(echo "$xml_required" | xpath "$xml_path[position()=$i]/groupId/text()" 2>/dev/null)
            ARTIFACTID=$(echo "$xml_required" | xpath "$xml_path[position()=$i]/artifactId/text()" 2>/dev/null)
            VERSION=$(echo "$xml_required" | xpath "$xml_path[position()=$i]/version/text()" 2>/dev/null)
            TYPE="$type"

            print_message info "-------------------------------------------------------------------------------"
            print_message info "groupId: $GROUPID"
            print_message info "artifactId: $ARTIFACTID"
            print_message info "version: $VERSION"
            print_message info "type: $type"
            print_message info "-------------------------------------------------------------------------------"
...
check_for_multi_module_project() {
    ...
    then
        ...
        # Getting total number of tags
        MODULE_COUNT=$(echo "$module_xml_required" | xpath "count(modules/module)" 2>/dev/null)
        ...
        do
            ...
            module_name=$(echo "$module_xml_required" | xpath "modules/module[position()=$p]/text()" 2>/dev/null)
